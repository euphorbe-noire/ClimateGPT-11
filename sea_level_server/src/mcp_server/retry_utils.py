...
# Import the existing logging setup
from src.utils.logging_setup import setup_logging

# Get the configured logger
logger = setup_logging('emissions_server')
...
def retry(exceptions: ExceptionType, tries: int = 4, delay: float = 3, 
    ...
    Args:
        ...
        logger: Logger to use. If None, the default logger is used.
    
    Returns:
        Decorated function that will retry on specified exceptions
    """
    # Use the default logger if none provided
    local_logger = logger if logger else setup_logging('emissions_server')
