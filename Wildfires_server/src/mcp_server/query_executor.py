...
def fix_date_formatting(sql: str) -> str:
    """
    Fix improperly formatted dates like '2015 - 01 - 30' to '2015-01-30'.
    """
    return re.sub(r"'(\d{4})\s*-\s*(\d{2})\s*-\s*(\d{2})'", r"'\1-\2-\3'", sql)
...
def execute_plan(steps: List[Dict[str, Any]], original_query: str) -> Dict[str, Any]:
    ...
    try:
        for i, step in enumerate(steps):
            ...
            sql_query = fix_date_formatting(clean_sql_query(step["sql"]))
