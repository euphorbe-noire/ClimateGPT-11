...
def format_forecast_explanation(data):
    ...
    first_year = forecast.get('years', [0])[0] if forecast.get('years') else 0
    last_year = forecast.get('years', [0])[-1] if forecast.get('years') else 0
    
    if not historical.get('values') or not forecast.get('values'):
        return "Could not generate forecast explanation due to missing data."
    
    last_hist = historical['values'][-1] if isinstance(historical['values'], list) else historical['values'].iloc[-1]
    ...
    forecast_trend = (forecast['values'][-1] - forecast['values'][0]) / (len(forecast['values']) - 1)
    ...
    # Feature information
    feature_info = ""
    if model.get('selected_features') and len(model['selected_features']) > 0:
...
### Forecast Summary
Based on {historical.get('years', [0])[0]}-{historical.get('years', [0])[-1]} data, the {model.get('type', 'ARIMA')} {model.get('order', '')} model predicts a **{abs(round(pct_change, 1))}%** {direction} in emissions by {last_year}, from {round(last_hist, 2)} to {round(last_forecast, 2)} units.
...
### Model Assessment
The optimal {model.get('type', 'ARIMA')}{model.get('order', '')} model was selected after comparing multiple parameter combinations. {feature_info}

### Reliability
- **Accuracy**: MAE of {metrics.get('MAE', 'N/A')} (average prediction error)
- **Confidence Interval**: By {last_year}, emissions expected between {round(forecast['lower_ci'][-1], 2)} and {round(forecast['upper_ci'][-1], 2)}
