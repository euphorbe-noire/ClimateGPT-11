...
"""
Visualization templates for ClimateGPT (adapted for wildfires).
...
if (pie_df[value_col] < 0).any():
    ...
    if not negative_values.empty:
        ...
        for i, row in negative_values.iterrows():
            print(f"  - {{row[label_col]}}: {{row[value_col]}}")
...
def get_scatter_plot_code(viz_spec):
    """Generate code for a scatter plot (useful for latitude/longitude plots)."""
    title = viz_spec["title"]
    x_axis = viz_spec["x_axis"]
    y_axes = viz_spec["y_axes"]
    columns = viz_spec["columns"]
...
# Create scatter plot
# If we have brightness or FRP data, use it for color coding
if 'brightness' in df.columns:
    scatter = plt.scatter(df["{columns[x_axis["index"]]}"], df["{columns[y_axes[0]["index"]]}"], 
                         c=df['brightness'], cmap='hot', s=50, alpha=0.6, edgecolors='black')
    cbar = plt.colorbar(scatter)
    cbar.set_label('Brightness (K)', fontsize=12)
elif 'frp' in df.columns:
    scatter = plt.scatter(df["{columns[x_axis["index"]]}"], df["{columns[y_axes[0]["index"]]}"], 
                         c=df['frp'], cmap='YlOrRd', s=50, alpha=0.6, edgecolors='black')
    cbar = plt.colorbar(scatter)
    cbar.set_label('Fire Radiative Power (MW)', fontsize=12)
elif 'confidence' in df.columns:
    scatter = plt.scatter(df["{columns[x_axis["index"]]}"], df["{columns[y_axes[0]["index"]]}"], 
                         c=df['confidence'], cmap='RdYlGn', s=50, alpha=0.6, edgecolors='black')
    cbar = plt.colorbar(scatter)
    cbar.set_label('Detection Confidence (%)', fontsize=12)
else:
    plt.scatter(df["{columns[x_axis["index"]]}"], df["{columns[y_axes[0]["index"]]}"], 
                color='red', alpha=0.6, s=50, edgecolors='black')
...
# If this is a geographic plot, set proper aspect ratio
if '{x_axis["label"]}' == 'longitude' and '{y_axes[0]["label"]}' == 'latitude':
...
def get_plot_code(viz_spec):
    ...
    if viz_type == "line":
        ...
        return get_scatter_plot_code(viz_spec)
